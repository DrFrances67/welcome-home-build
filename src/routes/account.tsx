...
function AccountPage() {
  ...
  const sectionTitle: React.CSSProperties = {
    ...
    letterSpacing: 0.5, color: "#64748b", margin: "24px 0 12px",
  };
  ...
  return (
    <main style={{ maxWidth: 640, margin: "0 auto", padding: "80px 20px 60px" }}>
      ...
      <p style={{ color: "#64748b", marginBottom: 24, fontSize: 14 }}>Update your profile information.</p>
      ...
      <form onSubmit={handleSave} style={{ background: "white", padding: 24, borderRadius: 12, boxShadow: "0 4px 20px rgba(0,0,0,0.05)", border: "1px solid #e2e8f0" }}>
        ...
        <div style={field}>
          <label style={label} htmlFor="homeAddress">Home address</label>
          <input id="homeAddress" style={input} value={homeAddress} onChange={(e) => setHomeAddress(e.target.value)} />
        </div>

        <div style={field}>
          <label style={label} htmlFor="schoolName">School name</label>
          <input id="schoolName" style={input} value={schoolName} onChange={(e) => setSchoolName(e.target.value)} />
        </div>

        <div style={field}>
          <label style={label} htmlFor="schoolAddress">School address</label>
          <input id="schoolAddress" style={input} value={schoolAddress} onChange={(e) => setSchoolAddress(e.target.value)} />
        </div>

        <div style={field}>
          <label style={label} htmlFor="schoolInfo">Additional school info</label>
          <textarea id="schoolInfo" style={{ ...input, minHeight: 90, resize: "vertical", fontFamily: "inherit" }}
            value={schoolInfo} onChange={(e) => setSchoolInfo(e.target.value)} />
        </div>
        ...
        <button type="submit" disabled={saving} style={{
          ...
          background: saving ? "#818cf8" : "#4f46e5", color: "white",
